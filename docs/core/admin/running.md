...
immediately regardless of the global setting. Note that with TLS the login
process keeps proxying the connection also after the login, so the login
service needs the same timeout as the service it hands the client to. A single
reload can override the setting for all the services with
...
[[setting,service_shutdown_clients_timeout]] also controls how long existing IMAP and
