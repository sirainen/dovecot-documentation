 ...
 * `shutdown_clients` was replaced by [[setting,service_shutdown_clients_timeout]].
   ...
   `shutdown_clients=no` becomes `service_shutdown_clients_timeout=infinite`. The
 ...
 * Only [[setting,service_type]] `client` and `login` services and the log
   ...
   [[setting,service_shutdown_clients_timeout]]. The internal services are always
 ...
 * The client-facing services now default to [[setting,service_type]] `client`:
   `doveadm`, `imap`, `imap-hibernate`, `imap-urlauth`, `imap-urlauth-worker`,
   `lmtp`, `managesieve`, `pop3` and `submission`. Unlike the other changed
   defaults this one applies also with an older
   [[setting,dovecot_config_version]], because the service type only tells the
   master process how the service's processes are handled.
